...
def evaluate_step(model, dataloader):
    ...
    for mel, lenx, indi_mel, cID, cID_type in dataloader:
        ...
        _nll, _indi_st_kl, _indi_dy_kl, _con_kl = model.loss_fn(outputs, mel, lenx)
        ...
        indi_st_kl += _indi_st_kl.item() * batch_size
        indi_dy_kl += _indi_dy_kl.item() * batch_size
  
    return nll / sample_size, indi_st_kl / sample_size, indi_dy_kl / sample_size, con_kl / sample_size
...
def log(logger, step=None, losses=None, fig=None, audio=None, sampling_rate=22100, tag="", model=None):
    # losses=[val_nll, val_con_kl, val_indi_st_kl, val_indi_dy_kl, val_loss])
    if losses is not None:
        logger.add_scalar("Loss/NLL", losses[0], step)
        logger.add_scalar("Loss/CON-KL", losses[1], step)
        logger.add_scalar("Loss/INDI-ST-KL", losses[2], step)
        logger.add_scalar("Loss/INDI-DY-KL", losses[3], step)
        logger.add_scalar("Loss/Total", losses[4], step)
...
def main(configs, file_config, experi_name):
    ...
    indi_st_gamma = train_config["optimizer"]["indi_st_gamma"]
    indi_st_mi = train_config["optimizer"]["indi_st_mi"]
    
    indi_dy_gamma = train_config["optimizer"]["indi_dy_gamma"]
    indi_dy_mi = train_config["optimizer"]["indi_dy_mi"]
    
    # Experiment name
    exp_name = '{}-{}-c_{}_{}-i_{}_{}'.format(
        model_name, experi_name.split('/')[-1], con_gamma, con_mi, \
                                                indi_st_gamma, indi_st_mi, \
                                                indi_dy_gamma, indi_dy_mi)
    ...
    while True:
        for mel, lenx, indi_mel, cID, cID_type in tqdm(trn_loader):
            ...
            indi_st_c = np.clip((indi_st_mi / stop_step) * global_step, 0, indi_st_mi)
            indi_dy_c = np.clip((indi_dy_mi / stop_step) * global_step, 0, indi_dy_mi)
            con_c = np.clip((con_mi / stop_step) * global_step, 0, con_mi)
            
            # total loss function
            loss = (nll + con_gamma * (con_kl - con_c).abs() +
                    indi_st_gamma * (indi_st_kl - indi_st_c).abs() + 
                    indi_dy_gamma * (indi_dy_kl - indi_dy_c).abs())
            ...
            if global_step > 0 and global_step % log_step == 0:
                losses = [nll.item(), con_kl.item(), indi_st_kl.item(), indi_dy_kl.item(), loss.item()]
                message1 = "Step {}/{}, ".format(global_step, total_step)
                message2 = "  NLL: {:.3f}, con-kl: {:.3f}, indi-st-kl: {:.3f}, indi-dy-kl: {:.3f}, loss: {:.3f}".format(*losses)
                message3 = "  gamma_con: {:.3f}, gamma_indi_st: {:.3f}, gamma_indi_dy: {:.3f}"\
                    .format((con_gamma * (con_kl - con_c).abs()).item(), \
                        (indi_st_gamma * (indi_st_kl - indi_st_c).abs()).item(), \
                        (indi_dy_gamma * (indi_dy_kl - indi_dy_c).abs()).item())
            ...
            if global_step > 0 and global_step % val_step == 0:
                model.eval()
                val_nll, val_indi_st_kl, val_indi_dy_kl, val_con_kl = evaluate_step(model, val_loader)
                val_loss = (val_nll + con_gamma * np.abs((val_con_kl - con_c)) + \
                            indi_st_gamma * np.abs((val_indi_st_kl - indi_st_c)) + \
                            indi_dy_gamma * np.abs((val_indi_dy_kl - indi_dy_c)))
                log(val_logger, step=global_step, model=model_name,
                    losses=[val_nll, val_con_kl, val_indi_st_kl, val_indi_dy_kl, val_loss])
                message = "Val-NLL: {:.3f}, val-con-kl: {:.3f}, val-indi-st-kl: {:.3f}, val-indi-dy-kl: {:.3f}, val_loss: {:.3f}"\
                    .format(val_nll, val_con_kl, val_indi_st_kl, val_indi_dy_kl, val_loss)
                message2 = "  Val_gamma_con: {:.3f}, val_gamma_indi_st: {:.3f}, val_gamma_indi_dy: {:.3f}"\
                    .format(con_gamma * np.abs(val_con_kl - con_c), \
                        indi_st_gamma * np.abs((val_indi_st_kl - indi_st_c)), \
                        indi_dy_gamma * np.abs((val_indi_dy_kl - indi_dy_c)))
                with open(os.path.join(val_log_path, "log.txt"), "a") as f:
                    f.write(message + "\n")
                print("\n", message + message2)
                val_losses.append([val_nll, val_indi_st_kl, val_indi_dy_kl, val_con_kl, val_loss])
...
if __name__ == "__main__":
    ...
    model_pathname = "model_greatbird_DS.yaml"
    train_pathname = "train_greatbird_DS.yaml" 
    ...
    # run
    experi_name = 'greatbird/greatbird_DS3'
