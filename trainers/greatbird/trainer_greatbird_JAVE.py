...
def evaluate_step(model, dataloader):
    ...
    for mel, lenx, indi_mel, cID, cID_type in dataloader:
        ...
        output_val = model(mel, lenx, indi_mel)
        Total_loss = model.loss_fn(output_val, mel, lenx, output_val['latent_dist'])
        _nll, _indi_kl, _con_kl, _indi_continuous_loss, _indi_discrete_loss = Total_loss['nll'], Total_loss['indi_kl'], Total_loss['con_kl'],  \
    ...
    while True:
        for mel, lenx, indi_mel, cID, cID_type in tqdm(trn_loader):
            ...
            if global_step > 0 and global_step % log_step == 0:
                ...
                message2 = "NLL: {:.3f}, con-kl: {:.3f}, indi-kl: {:.3f}, Total: {:.3f}".format(*losses)
                message3 = "gamma_con: {:.3f}, gamma_indi: {:.3f}"\
                    .format((con_gamma * (con_kl - con_c).abs()).item(), (indi_gamma * (indi_kl - indi_c).abs()).item())
                message4 = "    indi_continuous_loss: {:.3f}, indi_discrete_loss: {:.3f}".format(indi_continuous_loss, indi_discrete_loss)
                with open(os.path.join(train_log_path, "log.txt"), "a") as f:
                    f.write(message1 + message2 + message3 + message4 + "\n")
                print("\n", message1 + message2, "\n", message3 + message4)
                print('indi_gamma',indi_gamma,'indi_c',indi_c)
...
if __name__ == "__main__":
    ...
    model_pathname = "model_greatbird_JVAE.yaml"
    train_pathname = "train_greatbird_JVAE.yaml" 
    ...
    # run
    experi_name = 'greatbird/greatbird_JVAE2'
