...
def evaluate_step(model, dataloader):
    ...
    for mel, lenx, indi_mel, cID, cID_type in dataloader:
        ...
        _nll, _indi_loss, _con_kl = model.loss_fn(outputs, mel, lenx)
    ...
    return (nll / sample_size, indi_loss / sample_size, con_kl / sample_size)
...
def log(logger, step=None, losses=None, fig=None, audio=None, sampling_rate=22100, tag="", model=None):
    if losses is not None:
        ...
        logger.add_scalar("Loss/INDI-Loss", losses[2], step)
...
def main(configs, file_config, experi_name):
    ...
    # Experiment name
    exp_name = '{}-{}-c_{}_{}-i_{}'.format(
        model_name, experi_name.split('/')[-1], con_gamma, con_mi, indi_gamma)
    ...
    while True:
        for mel, lenx, indi_mel, cID, cID_type in tqdm(trn_loader):
            ...
            # total loss function
            loss = (nll + con_gamma * (con_kl - con_c).abs() + indi_gamma * indi_loss)
            ...
            if global_step > 0 and global_step % log_step == 0:
                losses = [nll.item(), con_kl.item(), indi_loss.item(), loss.item()]
                message1 = "Step {}/{}, ".format(global_step, total_step)
                message2 = "  NLL: {:.3f}, con-kl: {:.3f}, indi-loss: {:.3f}, loss: {:.3f}".format(*losses)
                message3 = "  gamma_con: {:.3f}, gamma_indi: {:.3f}"\
                    .format((con_gamma * (con_kl - con_c).abs()).item(), (indi_gamma * (indi_loss).abs()).item())
            ...
            if global_step > 0 and global_step % val_step == 0:
                model.eval()
                val_nll, val_indi_loss, val_con_kl = evaluate_step(model, val_loader)
                ...
                log(val_logger, step=global_step, model=model_name,
                    losses=[val_nll, val_con_kl, val_indi_loss, val_loss])
                message = "Val-NLL: {:.3f}, val-con-kl: {:.3f}, val-indi-loss: {:.3f}, val_loss: {:.3f}"\
                    .format(val_nll, val_con_kl, val_indi_loss, val_loss)
                message2 = "  gamma_con: {:.3f}, gamma_indi: {:.3f}"\
                    .format(con_gamma * np.abs((val_con_kl - con_c)), indi_gamma * np.abs(val_indi_loss))
                with open(os.path.join(val_log_path, "log.txt"), "a") as f:
                    f.write(message + "\n")
                print("\n", message + message2)
                val_losses.append([val_nll, val_indi_loss, val_con_kl, val_loss])
...
if __name__ == "__main__":
    ...
    model_pathname = "model_greatbird_Vector2.yaml"
    train_pathname = "train_greatbird_Vector2.yaml" 
    ...
    # run
    experi_name = 'greatbird/greatbird_Vector4'
