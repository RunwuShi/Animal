...
def evaluate_step(model, dataloader):
    ...
    indi_kl = 0.
    sample_size = 0
    for mel, lenx, indi_mel, _, cID_type in dataloader:
...
def main(configs, file_config, experi_name):
    ...
    trn_set = western_bird(dataset_config, subset='train') # 'train'
    val_set = western_bird(dataset_config, subset='val')
    ...
    # start training---------------------------------------------------------------
    while True:
        for mel, lenx, indi_mel, _, cID_type in tqdm(trn_loader):
            mel = mel.to(device)
            lenx = lenx.to(device)
            ...
            if global_step > 0 and global_step % val_step == 0:
                ...
                # reconstruction
                val_mel, val_lenx, val_indi_mel, _, val_cID_type = next(iter(val_single_sampler))
...
if __name__ == "__main__":
    ...
    # config file name
    dataset_pathname = "dataset_western_bird_chunk.yaml"
    model_pathname = "model_western_bird.yaml"
    train_pathname = "train_western_bird2.yaml" 
    
    # config path
    data_config_path  = "./Animal/configs/western_bird/" + "/" + dataset_pathname
    model_config_path = "./Animal/configs/western_bird/" + "/" + model_pathname
    train_config_path = "./Animal/configs/western_bird/" + "/" + train_pathname
    ...
    # run
    experi_name = 'western_bird/western_bird_3'
