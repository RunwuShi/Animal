...
def evaluate_step(model, dataloader):
    ...
    indi_kl = 0.
    sample_size = 0
    for mel, lenx, indi_mel, cID in dataloader:
...
def main(configs, file_config, experi_name):
    ...
    trn_set = vctkDataset(dataset_config, subset='train')
    val_set = vctkDataset(dataset_config, subset='val')
    tst_set = vctkDataset(dataset_config, subset='test')
    ...
    exp_name =  current_directory + '/' + "output" + '/'+ experi_name.split('/')[0] + '/' + exp_name
    ...
    # start training---------------------------------------------------------------
    while True:
        for mel, lenx, indi_mel, cID in tqdm(trn_loader):
            mel = mel.to(device)
            lenx = lenx.to(device)
            ...
            if global_step > 0 and global_step % val_step == 0:
                ...
                # reconstruction
                val_mel, val_lenx, val_indi_mel, val_cID_file, val_cID = next(iter(val_single_sampler))
                ...
                with torch.no_grad():
                    ...
                    log(val_logger, step=global_step, fig=gr_mel_fig,
                        tag="Val/step-{}-{}_mel_gt".format(global_step, val_cID))
                    log(val_logger, step=global_step, fig=rec_mel_fig,
                        tag="Val/step-{}-{}_mel".format(global_step, val_cID))
...
if __name__ == "__main__":
    ...
    # config file name
    dataset_pathname = "dataset_vctk.yaml"
    model_pathname = "model_vctk.yaml"
    train_pathname = "train_vctk.yaml" 
    
    # config path
    data_config_path  = "./Animal/configs/vctk/" + "/" + dataset_pathname
    model_config_path = "./Animal/configs/vctk/" + "/" + model_pathname
    train_config_path = "./Animal/configs/vctk/" + "/" + train_pathname
