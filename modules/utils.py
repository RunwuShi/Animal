...
class VectorQuantizerEMA(nn.Module):
    ...
    def __init__(self, device, num_embeddings, embedding_dim, commitment_cost=0.25, decay=0.999, epsilon=1e-5):
...
class VectorQuantizer(nn.Module):
    ...
    def __init__(self, n_embeddings, embedding_dim, beta, device): # # 512, 64, 0.25
        super(VectorQuantizer, self).__init__()
        self.n_e = n_embeddings
    ...
    def forward(self, z):
        ...
        d = torch.sum(z_flattened ** 2, dim=1, keepdim=True) + \
            ...
            torch.matmul(z_flattened, self.embedding.weight.t()) # [batch, n_embeddings]
