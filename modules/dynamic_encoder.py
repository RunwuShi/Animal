...
class dynamicEncoderBase_easy(nn.Module):
    def __init__(self, in_channels, h_channels, out_channels, conv_kernels, paddings, 
                 activation, sa_hidden, sa_layer, sa_head, sa_filter_size, sa_kernel_size,
                 sa_dropout, max_seq_len):
        super().__init__()
...
class dynamicEncoderBase_Vector(nn.Module):
    def __init__(self, in_channels, h_channels, out_channels, conv_kernels, paddings, 
                 activation, sa_hidden, sa_layer, sa_head, sa_filter_size, sa_kernel_size,
                 sa_dropout, max_seq_len):
        super().__init__()
    ...
    def forward(self, x, lens=None):
        ...
        # get distribution parameters
        out = self.mu_logs_linear(dec_output)
...
class indi_dynamicEncoderBase(nn.Module):
    def __init__(self, in_channels, h_channels, out_channels, conv_kernels, paddings, 
                 activation, sa_hidden, sa_layer, sa_head, sa_filter_size, sa_kernel_size,
                 sa_dropout, max_seq_len):
        super().__init__()
    ...
    def forward(self, x, lens=None):
        ...
        # convolution
        h = self.conv_initial(x) # [B, C, T]: C: eg 80->256, [1, 80, 443]->[1, 256, 443]
        for conv in self.conv_layers:
            h = conv(h, lens)
        h = self.activation(h)
        h = self.out_batch_norm(h, lens)
        ...
        # self-attention
        sa_input = h.permute(0, 2, 1)  # [B, C, T]->[B, T, C], [1, 256, 443]->[1, 443, 256]
        batch_size, max_len = sa_input.shape[0], sa_input.shape[1]
        ...
        if not self.training and sa_input.shape[1] > self.max_seq_len:
            ...
            slf_attn_mask = mask.unsqueeze(1).expand(-1, max_len, -1)
            dec_output = sa_input[:, :max_len, :] + self.position_enc[:, :max_len, :].expand(batch_size, -1, -1)
            mask = mask[:, :max_len] # [1, 443]
            slf_attn_mask = slf_attn_mask[:, :, :max_len] # [1, 443, 443]

        for dec_layer in self.layer_stack:
            dec_output, dec_slf_attn = dec_layer(
                dec_output, mask=mask, slf_attn_mask=slf_attn_mask)
        dec_output = dec_output.permute(0, 2, 1).contiguous() # [1, 443, 256]->[1, 256, 443]

        dec_output = torch.mean(dec_output, dim=[2])  # [batch, h_channels], ->[1, 256, 443]->[1, 256]
        
        # get distribution parameters
        mu_logs = self.mu_logs_linear(dec_output) # [batch, h_channels]->[batch, h_channels], [1, 256]->[1, 256]
        mu, log_std = torch.split(mu_logs, self.out_channels, dim=1) # [batch, h_channels]-># [batch, h_channels/2]
