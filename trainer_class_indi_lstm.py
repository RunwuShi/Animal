...
from datasets import MelDataset_lstm
...
def evaluate_step(model, dataloader):
    ...
    kld_z = 0.
    sample_size = 0
    for mel, lenx, ctID, cID in dataloader:
...
def main(configs, file_config, experi_name):
    ...
    trn_set = MelDataset_lstm(dataset_config, used_key = [[
                                                    #   'calltype_1',
                                                      'calltype_2',
                                                      'calltype_3',
                                                      'calltype_4',
    ...
    val_set = MelDataset_lstm(dataset_config, used_key = [[
                                                    #   'calltype_1',
                                                      'calltype_2',
                                                      'calltype_3',
                                                      'calltype_4',
    ...
    tst_set = MelDataset_lstm(dataset_config, used_key = [[
                                                    #   'calltype_1',
                                                      'calltype_2',
                                                      'calltype_3',
                                                      'calltype_4',
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        for mel, lenx, ctID, cID in tqdm(trn_loader):
            mel = mel.to(device)
            lenx = lenx.to(device)
            ...
            if global_step % val_step == 0:
                ...
                # reconstruction
                val_mel, val_lenx, val_ctID, val_cID = next(iter(val_single_sampler))
                val_mel = val_mel.to(device)
                val_lenx = val_lenx.to(device)
                
                with torch.no_grad():
                    outputs = model(val_mel) 
...
if __name__ == "__main__":
    ...
    model_pathname = "model_lstm.yaml"
